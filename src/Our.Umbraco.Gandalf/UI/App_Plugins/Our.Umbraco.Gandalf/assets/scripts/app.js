...
*/
angular.module("umbraco").controller("GandalfController", function ($scope, $filter, GandalfApi, ngTableParams) {
  ...
  */
    ...
    */
    $scope.fetchItems = function () {
        return GandalfApi.getAll().then($scope.onRecieveAllItemsResponse.bind(this));
    };
    ...
    */
    $scope.addItem = function (item) {
        GandalfApi.add(item.IpAddress, item.Notes)
    ...
    */
    $scope.updateItem = function (item) {
        GandalfApi.update(item).then($scope.onUpdateItemResponse.bind(this, item));
    }
    ...
    */
    $scope.deleteItem = function (item) {
        if (confirm("Are you sure you want to delete this item?")) {
            GandalfApi.remove(item.Id).then($scope.onDeleteItemResponse.bind(this, item));
        }
    }
...
*/
angular.module("umbraco.resources").factory("GandalfApi", function ($http) {
    return {
        //Get all from the server
        getAll: function () {
            return $http.get("backoffice/Gandalf/AllowedIpApi/GetAll");
        },
        //Send data to add a new
        add: function (ipAddress, notes) {
            return $http.post("backoffice/Gandalf/AllowedIpApi/Add", JSON.stringify({ ipAddress: ipAddress, notes: notes }));
        },
        //Send request to update an existing
        update: function (item) {
            console.log(item);
            return $http.post("backoffice/Gandalf/AllowedIpApi/Update", JSON.stringify({ Item: item }));
        },
        //Remove / Delete an existing 
        remove: function (id) {
            return $http.delete("backoffice/Gandalf/AllowedIpApi/Delete/" + id);
        },
